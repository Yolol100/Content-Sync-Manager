...
$assert(strpos($module, 'dca_tb_ai_image_context_private_meta_is_archival') !== false && strpos($module, "strpos(\$meta_key, '_dca_tb_') === 0") !== false, 'Content Sync archival metadata must be excluded from live private-media URL blockers.');
$assert(strpos($module, 'dca_tb_ai_image_context_attachment_url_targets') !== false, 'Private metadata scan must precompute full and intermediate attachment URL targets.');
$assert(strpos($module, "wp_get_attachment_image_src(\$attachment_id, \$size_name)") !== false, 'Private metadata safety targets must include generated/intermediate image URLs.');
...
$assert(strpos($module, "\$private_meta_targets = dca_tb_ai_image_context_attachment_url_targets(\$attachment_ids);") !== false, 'Site usage scan must compute private metadata URL targets only once per scan.');
